...
function handleSubmitClue(state, playerId, clue) {
    ...
    if (newRound.currentClueIndex >= round.clueOrder.length) {
        const summary = round.clueOrder.map((id) => {
...
function handleClueTimeout(state) {
    ...
    const sideEffects = [{ type: 'ANNOUNCE_PUBLIC', message: `⏰ ${timedOut?.name ?? '?'}: "..." (tiempo)` }];
...
function handleSubmitVote(state, playerId, votedId) {
    ...
    if (!aliveIds.includes(playerId)) return { state, sideEffects: [{ type: 'ANNOUNCE_PRIVATE', playerId, message: '❌ Estás fuera' }] };
    if (!aliveIds.includes(votedId)) return { state, sideEffects: [{ type: 'ANNOUNCE_PRIVATE', playerId, message: '❌ Jugador no válido' }] };
    if (playerId === votedId) return { state, sideEffects: [{ type: 'ANNOUNCE_PRIVATE', playerId, message: '❌ No puedes votarte a ti mismo' }] };
    ...
    const sideEffects = [{ type: 'ANNOUNCE_PUBLIC', message: `🗳️ ${voter?.name ?? '?'} votó (${newVotes.size}/${aliveIds.length})` }];
...
function handleEndVoting(state) {
    if ((state.phase !== types_1.GamePhase.VOTING && state.phase !== types_1.GamePhase.DISCUSSION) || !state.currentRound) return { state, sideEffects: [] };
    const round = state.currentRound;
    ...
    if (wasCorrect) {
        ...
        if (remainingInnocents.length <= 1) {
            ...
            sideEffects.push({ type: 'ANNOUNCE_PUBLIC', message: `💀 ${votedOut?.name} era INOCENTE. ¡El Impostor gana!`, style: { color: 0xff0000, style: 'bold', sound: 2 } });
            return { state: { ...state, phase: types_1.GamePhase.REVEAL, currentRound: { ...round, endedAt: Date.now(), result } }, sideEffects };
        } else {
            sideEffects.push({ type: 'ANNOUNCE_PUBLIC', message: `❌ ${votedOut?.name} era INOCENTE. El impostor sigue libre...`, style: { color: 0xffaa00, style: 'bold' } });
            ...
            const first = state.players.get(newClueOrder[0]);
            sideEffects.push({ type: 'ANNOUNCE_PUBLIC', message: `📝 NUEVA RONDA | Turno: ${first?.name ?? '?'}`, style: { color: 0x00bfff, style: 'bold', sound: 1 } });
            sideEffects.push({ type: 'SET_PHASE_TIMER', durationSeconds: state.settings.clueTimeSeconds });
...
function handleEndReveal(state) {
    ...
    if (result.impostorWon) {
        sideEffects.push({ type: 'ANNOUNCE_PUBLIC', message: `🔴 IMPOSTOR GANA | Era: ${result.impostorName} | Futbolista: ${result.footballer}`, style: { color: 0xff0000, style: 'bold', sound: 1 } });
    } else {
        sideEffects.push({ type: 'ANNOUNCE_PUBLIC', message: `🟢 JUGADORES GANAN | Impostor: ${result.impostorName} | Futbolista: ${result.footballer}`, style: { color: 0x00ff00, style: 'bold', sound: 1 } });
    }
...
function handleForceReveal(state) {
    ...
    return { state: { ...state, phase: types_1.GamePhase.RESULTS, currentRound: { ...round, result, endedAt: Date.now() }, roundHistory: [...state.roundHistory, result] }, sideEffects: [{ type: 'CLEAR_TIMER' }, { type: 'ANNOUNCE_PUBLIC', message: `⚠️ Admin reveló | Impostor: ${result.impostorName} | Futbolista: ${result.footballer}` }] };
}
...
function handleResetGame(state) {
    const sideEffects = [];
    if (state.queue.length >= state.settings.minPlayers) sideEffects.push({ type: 'AUTO_START_GAME' });
    return { state: { ...state, phase: types_1.GamePhase.WAITING, currentRound: null }, sideEffects };
}

function handleResetRound(state) {
    const sideEffects = [{ type: 'CLEAR_TIMER' }, { type: 'ANNOUNCE_PUBLIC', message: '⚠️ Ronda saltada', style: { color: 0xffaa00, sound: 2 } }];
    if (state.queue.length >= state.settings.minPlayers) sideEffects.push({ type: 'AUTO_START_GAME' });
    return { state: { ...state, phase: types_1.GamePhase.WAITING, currentRound: null }, sideEffects };
}
